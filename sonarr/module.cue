// Package sonarr defines the Sonarr TV series collection manager module.
...
//
// radarr is the same design with a different name, port and env prefix; the
// two are kept byte-identical apart from the delta documented in both READMEs.
//
// OPM v2 line: path major v1 (migrated 2026-09 from
// opmodel.dev/modules/sonarr@v2; this path starts its own line), core@v2
// identity reshape (complete modulePath with major, identity/ package, derived
// fqn), and the version-segment catalog imports (resources/v1beta1 etc.).
package sonarr

import (
	id "jacero.se/modules/sonarr/identity"
...
#storageVolume: {
	...
	// it is not for jellyfin: Sonarr's entire job is to MOVE files out of the
...
#config: {
	// Container image
	image: res.#Image & {
		repository: string | *"ghcr.io/home-operations/sonarr"
		tag:        string | *"4.0.15"
		digest:     string | *""
	}
	...
	//
	// This is how Servarr settings are set. Sonarr binds its configuration from
	// the environment as SONARR__<SECTION>__<KEY> — the image itself ships
	// SONARR__UPDATE__BRANCH=develop, which is what proves the binding is live
	...
	storage: {
		// Application data — SQLite (sonarr.db, logs.db) in WAL mode, so this
		...
		// Library directories, keyed by name (e.g. "series", "series4k").
		//
		...
		// The download client's completed-output directory. Sonarr reads from
