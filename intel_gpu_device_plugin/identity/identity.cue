// Package identity is the single source of this module's path and version
// (core #IdentityPackage). It sits at the bottom of the module's import graph
// — no intra-module imports, no core import; validation is external (a
// publishing tool unifies this package against core's #IdentityPackage).
package identity

// ModulePath is the module's complete CUE module path, major suffix included
// — byte-identical to cue.mod's `module:` field. Migrated 2026-09 from
// opmodel.dev/modules/intel_gpu_device_plugin@v2 (open-platform-model/modules,
// v2.0.3); this path starts its own line at v1.
ModulePath: "jacero.se/modules/intel_gpu_device_plugin@v1"

// Version is the module's bare SemVer; its major must agree with ModulePath's.
// A concrete literal, never a defaulted disjunction: the kernel's loader gate
// requires a value, and core's #IdentityPackage (which publish unifies this
// package against) supplies the SemVer constraint.
Version: "1.0.2"
